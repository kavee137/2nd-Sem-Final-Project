...
// Upload image function
    ...
    $('.toggle-password').on('click', function() {
        const passwordInput = $(this).siblings('.pass-input');
    ...
    $('form').on('submit', function(e) {
        ...
        const currentPassword = $('input.pass-input').eq(0).val();
        const newPassword = $('input.pass-input').eq(1).val();
        const confirmPassword = $('input.pass-input').eq(2).val();

        // Validate passwords
        if (!currentPassword || !newPassword || !confirmPassword) {
            showSweetAlert('error', 'All fields are required');
            return;
        }

        if (newPassword !== confirmPassword) {
            showSweetAlert('error', 'New passwords do not match');
            return;
        }

        // Password strength validation
        if (newPassword.length < 6) {
            showSweetAlert('error', 'Password must be at least 6 characters long');
            return;
        }

        // Send AJAX request to change password
        $.ajax({
            url: 'http://localhost:8082/api/v1/user/change-password',
            type: 'POST',
            contentType: 'application/json',
            data: JSON.stringify({
            ...
            success: function(response) {
                showSweetAlert('success', 'Password changed successfully');
                // Clear the form
    ...
    // Helper function to show Sweet Alert
